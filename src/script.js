document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("add-car").addEventListener("click", function () {
    if (document.getElementById("form-container").hasAttribute("hidden")) {
      document.getElementById("form-container").removeAttribute("hidden");
    } else {
      document.getElementById("form-container").setAttribute("hidden", true);
    }
  });
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (
      descriptionInput.value.length > 80 ||
      descriptionInput.value.length === 0
    ) {
      alert(
        "La descripció no pot ser més llarga de 80 caràcters i no pot estar buida."
      );
      return;
    }
    ...
    if (
      ...
      try {
        const deleteResponse = await fetch(
          `${apiUrl}/series/${seriesId}/cars/${carId}`,
          {
            method: "DELETE",
          }
        );
  ...
  function filterCarsByCategory(categoryId) {
    fetch(carsApiUrl)
      .then(response => response.json())
      .then(data => {
        dataContainer.innerHTML = '';
        const filteredCars = categoryId ? data.filter(car => car.seriesId === categoryId) : data;
